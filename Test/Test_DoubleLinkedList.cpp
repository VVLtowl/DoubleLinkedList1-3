...
#pragma region ========== データ数の取得 ==========
	...
	********************************************************/
	TEST(ListCountTest, WhenInsertToEndFailed)
	{
		...
		//末尾への挿入を失敗させる
		auto iter = otherList.End();//別リストのイテレータで失敗を起こす
		ScoreData data;
	...
	********************************************************/
	TEST(ListCountTest, WhenInsertFailed)
	{
		...
		//挿入を失敗させる
		auto iter = otherList.Begin();//別リストのイテレータで失敗を起こす
		ScoreData data;
		EXPECT_FALSE(list.Insert(iter, data));
...
#ifdef  GT_MANUAL_TEST_LIST
		const DoubleLinkedList<ScoreData> list;
		list.Remove(DoubleLinkedList<ScoreData>::Iterator());
#else
		SUCCEED();
#endif //  GT_MANUAL_TEST_LIST
	}
...
#pragma region ========== 先頭イテレータの取得 ==========
	...
	********************************************************/
	...
	{
#ifdef GT_MANUAL_TEST_LIST
		const DoubleLinkedList<ScoreData> constList;
		constList.Begin();
#else
		SUCCEED();
#endif
